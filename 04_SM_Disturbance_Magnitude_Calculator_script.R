...
dist_mag_calc <- function(ts, distyr, scrs){
  ...
  # d <- quantile(sc_series[,(dist-6):(dist-1)], c(0.01), na.rm = T)  # 1% quantile in 5yr pre-onset
  # d1 <- scrs[,dist] < d
  # d2 <- scrs[,dist+1] < d
  ...
  dmag_data <- cbind(series[,dist], series[,dist+1], steady, dmagy1, dmagy2, def1, def2)
  colnames(dmag_data) <- c(names(series[dist]), names(series[dist+1]), "steady", "dmag1", "dmag2", "dm_gt_0_y1", "dm_gt_0_y2")
  return(dmag_data)
}
